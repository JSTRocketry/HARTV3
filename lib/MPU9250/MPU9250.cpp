...
void MPU9250::calibrateAccel(){
  float xAccelBias = 0, yAccelBias = 0, zAccelBias = 0;
  for(int i = 0; i < 15; i ++){
    update();
    xAccelBias += runningData.accel.x;
    yAccelBias += runningData.accel.y;
    zAccelBias += runningData.accel.z - 1.0;
    delay(10);
  }
  accelCalibrationOffsets[0] = xAccelBias/15.0;
  accelCalibrationOffsets[1] = yAccelBias/15.0;
  accelCalibrationOffsets[2] = zAccelBias/15.0;
  /*
  for(int i = 0; i < 3; i ++){
...
void MPU9250::zero(){
  ...
  //accelCalibrationOffsets[0] = data.accel.x;
  //accelCalibrationOffsets[1] = data.accel.y;
  //accelCalibrationOffsets[2] = 1 - data.accel.z;
  //orientationOffsets[2] = data.orientation.z;
}

void MPU9250::resetVelocityDistanceStats(){
  runningData.velocity.x = 0;
  runningData.velocity.y = 0;
  runningData.velocity.z = 0;
  runningData.distance.x = 0;
  runningData.distance.y = 0;
  runningData.distance.z = 0;
}
...
float cosDegrees(float degrees){
  return cos(degrees/RADIANS_TO_DEGREES);
}

float sinDegrees(float degrees){
  return sin(degrees/RADIANS_TO_DEGREES);
}
float by2;
float bz2;
float bx3;
void MPU9250::tiltCompensateMagnetometer(MPU9250_Scaled_Data *scaleData){
    by2 = scaleData->mag.z * sinDegrees(runningData.orientation.y) - scaleData->mag.y * cosDegrees(runningData.orientation.y);
    bz2 = scaleData->mag.y *sinDegrees(runningData.orientation.y) + scaleData->mag.z* cosDegrees(runningData.orientation.y);
    bx3 = scaleData->mag.x *cosDegrees(runningData.orientation.x) + bz2 * sinDegrees(runningData.orientation.x);
    //Serial.println("Mag X: " + String(scaleData->mag.x) + " Normalized: " + String(bx3));
    //tilt compensation
    ...
    //Serial.println("Mag Heading: " + String(headingMT) + " Normalized Heading: " + String(atan2(by2,bx3)*RADIANS_TO_DEGREES));
...
void MPU9250::update(){
  ...
  updateVelocityDistance(tempTime - timeAtLastRead);
  timeAtLastRead = tempTime;
}

void MPU9250::updateVelocityDistance(long elapsedTime){
  runningData.velocity.x += runningData.accel.x * elapsedTime/1000000.0f;
  runningData.velocity.y += runningData.accel.y * elapsedTime/1000000.0f;
  runningData.velocity.z += (runningData.accel.z - 1.0) * elapsedTime/1000000.0f;

  runningData.distance.x += .5 * runningData.accel.x * pow(elapsedTime/1000000.0f,2);
  runningData.distance.y += .5 * runningData.accel.y * pow(elapsedTime/1000000.0f,2);
  runningData.distance.z += (.5 * runningData.accel.z - 1.0) * pow(elapsedTime/1000000.0f,2);
...
void MPU9250::scaleData(MPU9250_Raw_Data *raw, MPU9250_Scaled_Data *data){
  ...
  data->accel.x = raw->accel.x / accelScale - accelCalibrationOffsets[0];
  data->accel.y = raw->accel.y / accelScale - accelCalibrationOffsets[1];
  data->accel.z = raw->accel.z / accelScale - accelCalibrationOffsets[2];
  //Serial.println("Z accel calibration offset: " + String(accelCalibrationOffsets[2]));
